...
export function BannerTextCard({ imageUrl, title, content, position = 'left' }: CardProps) {
  return (
    <Animation className="relative w-full h-[260px] md:h-[360px] lg:h-[420px]  overflow-hidden shadow-8">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          className={cn(
            'absolute inset-y-0 w-full md:w-[54%]',
      ...
      <div
        className={cn(
          'absolute inset-y-0 flex items-center p-5 md:p-10',
          position === 'right' ? 'right-0 justify-end' : 'left-0'
        )}
      >
        <div className="max-w-[720px] text-white">
          {/* bullet circle + title */}
          <div className="flex items-center gap-3 mb-3">
            <span className="inline-flex items-center justify-center w-6 h-6 rounded-full border-2 border-white/80 bg-white">
          ...
          <p className="text-white/90 text-sm md:text-base leading-relaxed max-w-[550px] whitespace-pre-wrap">
            {content}
          </p>
        </div>
      </div>
